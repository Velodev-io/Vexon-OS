    ...
    (set) => ({
      ...
      addEvent: (eventData) => set((state) => {
        ...
        if (event.type === 'stream_token') {
          ...
          const lastCall = toolCalls.find((toolCall: any) => toolCall.tool === event.data.tool && toolCall.status === 'running')
          if (lastCall) {
      ...
      setActiveSession: (sessionId) => set({ activeSessionId: sessionId, events: [], activeAgents: {} }),
    }),
    {
